<?php
	session_start();
	include"../inc/info/myinfo-inc.php";
...
<html>
	<head>
		<!-- Title Page-->
		<title>ICCES | My Student Information</title>
        <meta charset="utf-8"/> 
...
<!------------------------------------------------------------------------------------->
							...
							<div class="row">
								...
								<div class="col-6 form-group">
									<div class="input-group">
										<div class="input-group-addon">
											<i class="fas fa-bookmark"></i> Curriculum Type
										</div>
...
<!------------------------------------------------------------------------------------->
                        <form method="post">
<?php 
$stmt100 = $bruh->query('SELECT studentinfoid FROM table_student_req WHERE studentinfoid = "'.$_SESSION['infoid'].'"');
$result = $stmt100->fetch(PDO::FETCH_ASSOC);
		if (!is_array($result))  { 
?>
		No Requisites Created.
<?php 	} elseif($result['studentinfoid'] == $_SESSION['infoid']) { ?>
							<div class="row">
								<div class="col-6 form-group">
									<div class="input-group">
										<div class="input-group-addon">
...
<!------------------------------------------------------------------------------------->
                        <form method="post">
<?php 
$stmt101 = $bruh->query('SELECT studentinfoid FROM table_student_cor WHERE studentinfoid = "'.$_SESSION['infoid'].'"');
