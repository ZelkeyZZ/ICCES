...
//===========================================================================================================================
	...
	if(empty($studentinfoid_err)){
		...
		$dup = "SELECT * FROM table_student_req LEFT JOIN table_student_info ON table_student_info.id = table_student_req.studentinfoid WHERE table_student_req.studentinfoid = '".$_POST["studentinfoid"]."'";
		...
		if (!$row) {
			$sql = "INSERT INTO table_student_req (studentinfoid) VALUES (:studentinfoid)";
			...
			if($stmt = $pdo->prepare($sql)){
				...
				if($stmt->execute()){
					header("location: ../404/reqinfosuccess.php");
					exit();
				} else{
			...
			header("location: ../404/reqinfo404.php");
            exit();
		}
    }
